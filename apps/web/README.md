...
| | 화면 체험 (`/tour`) | 온체인 데모 (로그인 후 화면) |
|---|---|---|
| 필요한 것 | 웹 서버만 (API·DB·체인·지갑 불필요, `next start`/정적 호스팅 가능) | 아래 1장의 전체 스택 + 역할별 EVM 지갑 |
| 데이터 | 브라우저 안의 고정 가상 데이터·가상 잔액 | PostgreSQL + 로컬 EVM 계약의 실제 상태 |
| 서명 | **간편 승인** 버튼 (시뮬레이션) | 각 조직 승인 지갑의 실제 서명 |
| 표시 | 보라색 "시뮬레이션 · 실제 거래 없음" 띠 고정. 거래 해시·체인 확정·DB 반영 문구를 **절대 표시하지 않음** | 준비→서명→전송→체인 확정→DB 반영, 실제 해시 |
| 검증 | `npm run e2e:tour` (웹 서버만 필요) | `npm run e2e:web` |

로그인 화면 아래 "화면만 체험하기"의 세 버튼(납품업체/구매처/은행으로 체험하기)으로 들어간다. 한 거래(300만원 채권 → 297만원 먼저받기 → 300만원 상환)를 역할 화면을 바꿔 가며 9번의 버튼으로 끝낸다. 자기 차례가 아니면 진행 버튼 대신 "○○ 화면으로 전환"이 나온다. 진행 상태는 브라우저 localStorage에만 있고 "처음부터"로 초기화한다. 서류 분석 표는 **예시**이며 OCR·LLM을 실행하지 않는다. 구현: `src/tour/model.ts`(순수 상태 기계, 단위 테스트), `src/app/tour/page.tsx`.
